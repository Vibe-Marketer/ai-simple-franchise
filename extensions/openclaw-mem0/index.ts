...
class OSSProvider implements Mem0Provider {
  ...
  private async _init(): Promise<void> {
    ...
    // Fix: mem0ai's OpenAIEmbedder constructor doesn't pass baseURL to the
    // OpenAI SDK client, so custom API endpoints (e.g. OpenRouter) are ignored.
    // Monkey-patch the baseURL on the embedder's client after initialization.
    const embedderBaseURL =
      (this.ossConfig?.embedder as any)?.config?.baseURL ||
      (this.ossConfig?.embedder as any)?.config?.url;
    if (embedderBaseURL && (this.memory as any).embedder?.openai) {
      (this.memory as any).embedder.openai.baseURL = embedderBaseURL;
    }
  }
